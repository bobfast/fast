...
)
{
	...
	if (Win32Protect == PAGE_EXECUTE_READWRITE) {
		...
		if (!ReadProcessMemory(GetCurrentProcess(), BaseAddress, &realBaseAddr, sizeof(realBaseAddr), &readbyte)) {
			...
			sprintf_s(buf, "%lu:%lu:%016llx:%08lx:%s*CallNtMapViewOfSection:",
				GetCurrentProcessId(), target_pid, realBaseAddr,
				(DWORD)CommitSize, szImagePath);
			printStack(buf);
			EnterCriticalSection(&api_cs);
...
//#####################################
// Hook CreateRemoteThread Hooking Function
...
//#####################################
// Hook VirtualAllocEx Hooking Function
...
//#####################################
// Hook WriteProcessMemory Hooking Function
	...
	SIZE_T* lpNumberOfBytesWritten)
{
	...
	sprintf_s(buf, "%lu:%lu:%016llx:%08lx:%s*CallWriteProcessMemory:Not Monitoring.*", GetCurrentProcessId(), target_pid, (DWORD64)lpBaseAddress, (DWORD)nSize, szImagePath);
	//printStack(buf);
