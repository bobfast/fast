...
void CallSetThreadContext(LPVOID monMMF) {
	...
	if (item != rwxList.end()) {
		...
		for (auto i : item->second) {
			if (i.first <= target && (i.first + (DWORD64)i.second > target)) {
				sprintf_s(buf, "%s:Detected:%016llx:CallSetThreadContext", caller_pid.c_str(), target);
				form->logging(gcnew System::String(caller_pid.c_str()));
				form->logging(gcnew System::String(" : "));
				form->logging(gcnew System::String(callee_pid.c_str()));
				...
				memory_region_dump(std::stoi(callee_pid), "MemoryRegionDump_SetThreadContext", rwxList);
				memcpy(monMMF, buf, strlen(buf));
				return;
			}
		}
	...
	sprintf_s(buf, "%s:%016llx:CallSetThreadContext:Clean", caller_pid.c_str(), target);
	memcpy(monMMF, buf, strlen(buf));
}
